...
/*   Updated: 2024/01/11 17:10:55 by bautrodr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void fill_init_env_list(t_paths *paths, char **envp)
{
    ...
    while (envp[i])
    {
        name = ft_substr(envp[i], 0, ft_strlen(envp[i]) - ft_strlen(ft_strchr(envp[i], '=') + 1));
		value = ft_substr(envp[i], ft_strlen(envp[i]) - ft_strlen(ft_strchr(envp[i], '=')) + 1, ft_strlen(envp[i]) );
